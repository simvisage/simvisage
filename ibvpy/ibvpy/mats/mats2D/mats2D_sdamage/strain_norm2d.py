...
class Mises(IStrainNorm2D):
    ...
    def get_f_trial(self, epsilon, D_el, E, nu, kappa):
        ...
        t20 = pow(epsilon[2], 2.)
        t26 = pow((1 + nu), 2.)
        t31 = sqrt((t10 / t11 * t14) + 12. * self.k * (t16 /2. + t18 /2. + t20 / 4. - t14 / 6.) / t26)
    ...
    def get_dede(self, epsilon, D_el, E, nu):
        ...
        t26 = pow( (1. + nu), 2.)
        t27 = 1. / t26
        t31 = sqrt((t12 * t14) + 12. * self.k * (t16 / 2. + t18 / 2. + t20 / 4. - t14 / 6.) * t27)
        t32 = 1. / t31
        t33 = t2 * t32
        t35 = 2. * t12 * t13
        dede[0] = t8 + t33 * ( t35 + 12. * self.k * (2. / 3. * epsilon[0] - epsilon[1] / 3.) * t27) / 4.
        dede[1] = t8 + t33 * ( t35 + 12. * self.k * (2. / 3. * epsilon[1] - epsilon[0] / 3.) * t27) / 4.
...
class Rankine(IStrainNorm2D):
    ...
    '''    
    def get_f_trial(self, epsilon, D_el, E, nu, kappa):
        sigma_I = linalg.eigh(map2d_sig_eng_to_mtx(dot(D_el,epsilon)))[0]#main stresses
        eps_eqv = linalg.norm(where(sigma_I >= 0., sigma_I, zeros(2)))/E#positive part and norm
    ...
    def get_dede(self, epsilon, D_el, E, nu):
        ...
        t66 = D_el[0][1] / 2.
        t67 = D_el[1][1] / 2.
        t85 = t37 * (2. * t6 * D_el[0][1] - 2. * t6 * D_el[1][1] + 2. * t16 * epsilon[1] - 2. * D_el[0][1] * D_el[1][0] * epsilon[0] - 4. * t7 * D_el[1][1] + 2. * t19 * D_el[1][1] + 2. * t29 * epsilon[1]) / 4.
...
class Mazars(IStrainNorm2D):
    ...
    def get_dede(self, epsilon, D_el, E, nu):
        ...
        t100 = t16 * t99
        dede[0] = t33 * (t43 * t25 + t15 * (t45 * D_el[0][0] + t52 * D_el[1][0]) + t51 * t29 + t22 * (t45 * D_el[0][1] + t52 * D_el[1][1])) / 2.
        dede[1] = t33 * (t70 * t25 + t15 * (t72 * D_el[0][0] + t78 * D_el[1][0]) + t77 * t29 + t22 * (t72 * D_el[0][1] + t78 * D_el[1][1])) / 2.
        dede[2] = t33 * (t93 * t25 + t15 * (t95 * D_el[0][0] + t100 * D_el[1][0]) + t99 * t29 + t22 * (t95 * D_el[0][1] + t100 * D_el[1][1])) / 2.
        return dede
