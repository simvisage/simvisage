...
class FETS1D52L6ULRH(FETSEval):
    ...
    def get_N_mtx(self, r_pnt):
        ...
        r = r_pnt[0]
        s = r_pnt[1]
        N_bar_mtx = array([1. / 2 * r * (r - 1.), 1. - r * r, 1. / 2 * r * (r + 1.)], dtype = float_)
    ...
    def get_B_mtx(self, r, X):
        ...
        # 
        cx = array(self.geo_r, dtype = float_)
        N = array([1. / 2. * (r[0] ** 2 - r[0]), 1. - r[0] ** 2 , 1. / 2. * (r[0] ** 2 + r[0])], dtype = float_)
        dN = array([(2 * r[0] - 1.) / L, -4. * r[0] / L, (2 * r[0] + 1.) / L], dtype = float_)
        ...
        B_mtx = array([[ dN[0], 0, dN[2], 0, 0, 0, 0, 0, dN[1], 0, 0, 0], # eps_1
                       [ N[0], 0, N[2], 0, -N[2], 0, -N[0], 0, N[1], 0, -N[1], 0], # slip
                       [    0, N[0], 0, N[2], 0, -N[2], 0, -N[0], 0, N[1], 0, -N[1]], # opening
