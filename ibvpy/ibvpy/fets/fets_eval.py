...
class FETSEval(TStepperEval):
    ...
    def map_r2X(self, r_pnt, X_mtx):
        ...
        '''
        #print "mapping ",dot( self.get_N_geo_mtx(r_pnt)[0], X_mtx )," ", r_pnt
        return dot(self.get_N_geo_mtx(r_pnt)[0], X_mtx)
...
#            else:
    ...
    def get_mtrl_corr_pred(self, sctx, eps_eng, d_eps_eng, tn, tn1, eps_avg = None):
        if self.mats_eval.initial_strain:
            ...
            eps_eng -= eps0_eng
        if eps_avg != None:
            sig_mtx, D_mtx = self.mats_eval.get_corr_pred(sctx, eps_eng, d_eps_eng, tn, tn1, eps_avg)
        else:
            sig_mtx, D_mtx = self.mats_eval.get_corr_pred(sctx, eps_eng, d_eps_eng, tn, tn1,)
    ...
    _GP_COORDS = [[0.], # for polynomial order  = 0 
                   ...
                   [-0.57735026918962584, 0.57735026918962584], # por polynomial order = 2
                   [-0.7745966692414834, 0., 0.7745966692414834], # for polynomial order = 3
                   [-0.861136311594053, -0.339981043584856, 0.339981043584856, 0.861136311594053] # for polynomial order = 4
                   ]

    _GP_WEIGHTS = [[1.], #
                   [2.], #
                   [1., 1.], #
                   [0.55555555555555558, 0.88888888888888884, 0.55555555555555558],
    ...
    def get_eps0_mtx33(self, sctx, u):
        ...
        if self.mats_eval.initial_strain:
            X_pnt = self.get_X_pnt(sctx)
            x_pnt = self.get_x_pnt(sctx)
            eps0_mtx33[ self.dim_slice, self.dim_slice ] = self.mats_eval.initial_strain(X_pnt, x_pnt)
        return eps0_mtx33
