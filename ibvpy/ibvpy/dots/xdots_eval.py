...
#        sctx.r = fets_eval.map_to_local( sctx.elem, sctx.X )
    ...
    def _get_rt_triangles( self ):
        ...
        for pos_r, neg_r, i_r in  zip( self.elem_r_contours[0],
            ...
            if i_r.shape[1] == 1:
                ...
                dir_vct = i_r[1] - i_r[0]#direction vector of the intersection
                norm_vct = array( [-dir_vct[1], dir_vct[0]], dtype = float )#normal_vector 
                shift_vct = norm_vct / linalg.norm( norm_vct ) * self.sdomain.rt_tol
                # ???
                ...
                shift_rs_pts = vstack( ( shift_rs, shift_rs ) )#TODO:make for arbitrary number of i_r
            ...
            if dot( pos_dir, norm_vct ) > 0.:#check that the normal points out from the crack
    ...
    def _get_ip_X( self ):
        ...
        for X_mtx, ip_addr0, ip_addr1  in zip( self.sdomain.elem_X_map,
            ...
            ip_X.append( self.fets_eval.get_vtk_r_glb_arr( X_mtx, self.ip_coords[ip_slice] ) )
        return vstack( ip_X )
    ...
    def _get_disc_integ_structure( self ):
        ...
        ip_wei_list = []
        ip_offset = 0
        id_map = array( [arange( self.sdomain.fe_grid_slice.r_i.shape[1] )], dtype = int )
        ...
        for segment in self.sdomain.fe_grid_slice.r_i:
            ip_coords = self.fets_eval.get_ip_coords( [segment, id_map], self.fets_eval.nip_disc )
            ip_offset += ip_coords.shape[0]
            ip_off_list.append( ip_offset )
            ip_coo_list.append( ip_coords )
            ip_wei_list.append( self.fets_eval.get_ip_weights( [segment, id_map], self.fets_eval.nip_disc ) )
...
#        return array(ip_off_list , dtype = int),array([ip_coord],dtype=float),array([2.],dtype=float)
    ...
    def _get_ip_ls_values( self ):
        ls_fn = frompyfunc( self.sdomain.ls_fn_X, 2, 1 )#TODO:define the ineraction wirh ls
    ...
    def _get_ip_normal( self ):
        ...
        if ir_shape == 1:#1D
            if self.dof_node_ls_values[0, 0] < 0.:#assuming that the first node is the left-most
                normal = [1.]
            else:
                normal = [-1.]
        elif ir_shape == 2:#2D
    ...
    vtk_X = Property( depends_on = 'sdomain.changed_structure, sdomain.+changed_geometry' )
    @cached_property
    def _get_vtk_X( self ):
        ...
        for triangle, e in zip( self.rt_triangles, self.sdomain.elements ):
            X_mtx = e.get_X_mtx()
            vtk_X.append( self.fets_eval.get_vtk_r_glb_arr( X_mtx, triangle[0][:, :2] ) )#TODO:slicing works just for 2D
        #print 'vtk_X ',vtk_X
        #return array(vtk_X)
...
#                     for triangle in self.rt_triangles])
    ...
    vtk_ls_values = Property( depends_on = 'sdomain.changed_structure, sdomain.+changed_geometry' )
    @cached_property
    ...
    def _apply_on_ip_pnts( self, fn ):
        ...
        out_single = fn( self.ip_coords[0], X_el[0], self.dof_node_ls_values[0], self.ip_ls_values[0] )
    ...
    def _apply_on_ip_disc( self, fn ):
        ...
        # test call to the function with single output - to get the shape of the result.        
        out_single = fn( self.ip_disc_coords[0], X_d[0], X_el[0], self.dof_node_ls_values[0], self.ip_normal[0] )
    ...
    X_i = Property( depends_on = 'sdomain.changed_structure, sdomain.+changed_geometry' )
    @cached_property
    def _get_X_i( self ):
        '''
    ...
    def _get_state_array( self ):
        ...
        for e_id, elem in enumerate( self.sdomain.elements ):
            sctx.elem = elem
            sctx.elem_state_array = state_array[ self.ip_offset[e_id] * mats_arr_size\
                                                : self.ip_offset[( e_id + 1 )] * mats_arr_size ]
            #print 'elem_state_array ', sctx.elem_state_array
            self.fets_eval.setup( sctx, ( self.ip_offset[( e_id + 1 )] - self.ip_offset[e_id] ) )

        # Transfer the values from the old state array - if an old_array was there
        if len( self.old_state_array ):

            # get the start-end indices of the elements in the old grid
            old_start_arr = self.old_state_start_elem_grid[ self.old_state_grid_ix ]
    ...
    def get_elem_state_array( self, e_id ):
        ...
        '''
        mats_arr_size = self.fets_eval.m_arr_size
        return self.state_array[ self.ip_offset[e_id] * mats_arr_size\
                                : self.ip_offset[( e_id + 1 )] * mats_arr_size ]
    ...
    def get_corr_pred( self, sctx, u, du, tn, tn1, F_int ):
        ...
        for e_id, elem, ip_addr0, ip_addr1, ip_disc_addr0, ip_disc_addr1  in zip( self.sdomain.idx_active_elems,
            ...
            sctx.elem_state_array = state_array[ self.ip_offset[e_id] * mats_arr_size\
                                                : self.ip_offset[( e_id + 1 )] * mats_arr_size ]#differs from the homogenous case
            sctx.X = elem.get_X_mtx()
            sctx.x = elem.get_x_mtx()
            if self.cache_geo_matrices:
                Be_mtx_grid = B_mtx_grid[ ip_slice, ... ]#differs from the homogenious case
