...
class DOTSEvalAvg( DOTSEval ):
    ...
    def setup( self, sctx ):
        ...
        for elem in sctx.sdomain.elements:
            sctx.elem = elem
            id = elem.id_number
            sctx.elem_state_array = sctx.state_array[ id * e_arr_size : ( id + 1 ) * e_arr_size ]
        ...
        for n_nod_i in node_list:
            ...
            #initialisation of all the values that are calculated on the fly
...
#           
            ...
            for elem_j in active_elems:
                ...
                for gp in self.fets_eval.gp_list:
                    ...
                    #value of bell function centered in node evaluated in Gauss point
...
#                   #Evaluate r_00, r_10 and R_11
                        ...
                        r_10_tot += gp[1] * J_det * alpha * ( x_pnt - self.avg_function.center )[:, self.fets_eval.dim_slice]
            ...
            if self.correction == True:
                ...
                A = vstack( [hstack( [r_00_tot, r_10_tot] ), hstack( [r_10_tot.T, R_11_tot] )] )
                ...
                #if we don't want the correcting factors the weight functionhas to be normed
                p_0 = 1. / r_tot
                p_1 = zeros( dim )
            ...
            for elem_j in active_elems:
                ...
                for gp in self.fets_eval.gp_list:
                    ...
                    c += r_00[0] * ( p_0 + dot( p_1, ( ( x_pnt - self.avg_function.center )[:, self.fets_eval.dim_slice] ).T ) ) * N_mtx
                    r_00.pop( 0 )
    ...
    def get_corr_pred( self, sctx, u, du, tn, tn1 ):
        ...
        for elem in sctx.sdomain.elements:
            ...
            f, k = self.fets_eval.get_corr_pred( sctx, u[ix_( ix )], du[ix_( ix )], tn, tn1, u_avg[ix_( ix )] )
            self.K[ ix_( ix, ix ) ] += k
            self.F_int[ ix_( ix ) ] += f
