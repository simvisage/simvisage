...
from numpy import \
    ...
    logical_and, argwhere, ix_, ones_like, fabs
...
class XFESubDomain( FESubDomain ):
    ...
    def _get_dof_enum_data( self ):
        ...
        #
        elem_dof_map = copy( dofs ).reshape( sh[0], self.fets_eval.parent_fets.n_e_dofs )

        elem_xdof_map = ( copy( dofs )[:, :, self.fets_eval.x_slice] ).reshape( sh[0], n_xdofs )
