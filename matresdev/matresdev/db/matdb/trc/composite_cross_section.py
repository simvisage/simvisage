...
class CompositeCrossSection(SimDBClass):
    ...
    def _get_thickness_arr(self):
        ...
        '''
        return array([flu.thickness for flu in self.fabric_layup_list ], dtype='float_')
    ...
    def _get_rho_arr(self):
        ...
        '''
        return array([flu.rho for flu in self.fabric_layup_list ], dtype='float_')
    ...
    def _get_E_tex_arr(self):
        ...
        '''
        return array([flu.E_tex for flu in self.fabric_layup_list ], dtype='float_')

    E_tex = Property(Float, unit='MPa', depends_on='input_change')
    @cached_property
    def _get_E_tex(self):
        '''get the larges value of the textile reinforcement for the plot of the analytical stiffness
        in 'exp_browse'
        '''
...
#        return sum( self.E_tex_arr * self.thickness_arr ) / self.thickness
    ...
    def get_E_c_time(self, age):
        ...
        return sum(((1 - rho_arr) * E_m_arr + rho_arr * E_tex_arr) * self.thickness_arr) / self.thickness
    ...
    def set_param(self, material_model, calibration_test, df):
        ...
        # and store it
        #
        key = cm_key + '_' + flo_key + '_' + '%.5f' % (s_tex_z) + '_' + orientation_fn_key
...
if __name__ == '__main__':
    ...
    from etsproxy.traits.api import Bool, TraitError
