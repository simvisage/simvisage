...
#         xdata, ydata = np.loadtxt("E:/Ehsan/Abaqus Modelling/Microplane/Conventional Shell/Tonschale/Simulation with Slack/E-S Data/Shorter/4fy50.txt").T
#         xdata = xdata / 1000.0
...
#            mats_state_array = copy(self.tloop.tstepper.sctx.mats_state_array)
            ...
            axes.plot(x, y, color='blue', linewidth=0.5)
...
#        font.monospace     : Courier, Computer Modern Typewriter
        ...
        for n in range(self.n_steps):
            for i in range(len(phi_trial_list_n[n + 1])):
                ...
                p.subplot(222)
                p.plot(eps, sig_trail, color='k', linewidth=linewidth)
                ...
                p.plot(eps, phi_trail, color='k', linewidth=linewidth)
                p.xlabel(r'strain $\varepsilon$ [1E-3]', fontproperties=font)
        ...
        p.plot(eps, sig_target, color='black', linewidth=linewidth)
        p.xlabel(r'strain $\varepsilon$ [1E-3]', fontproperties=font)
        p.ylabel('stress $\sigma$ [MPa]', fontproperties=font)
        if self.format_ticks:
            # format ticks for plot
        ...
        p.plot(eps, phi_fn, color='black', linewidth=linewidth)
        p.xlabel(r'strain $\varepsilon$ [1E-3]', fontproperties=font)
        p.ylabel('integrity $\phi$ [-]', fontproperties=font)
        if self.format_ticks:
            # format ticks for plot
...
#                               'tensile_tests',
#                              'buttstrap_clamping',
#                              '2013-06-18_TTb-6c-2cm-TU_bs3-Aramis2d',
#                              'TTb-6c-2cm-90-TU-V2_bs3.DAT')
