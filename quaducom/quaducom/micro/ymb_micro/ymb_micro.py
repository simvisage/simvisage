...
class TitleHandler(Handler):
    ...
    def object_yarn_type_changed(self, info):
        ...
        object.
        """
        info.ui.title = '%s -- Yarn-Matrix-Bond Microstructure Lab' % \
            info.object.data.source.yarn_type
...
class YMBMicro(HasTraits):
    ...
    auto_correl_data = Property(
        Instance(YMBAutoCorrel), depends_on='data.input_change')

    @cached_property
    def _get_auto_correl_data(self):
        return YMBAutoCorrel(data=self.data)

    auto_correl = Property(
        Instance(YMBAutoCorrelView), depends_on='data.input_change')

    @cached_property
    def _get_auto_correl(self):
        return YMBAutoCorrelView(correl_data=self.auto_correl_data)

    cross_correl = Property(
        Instance(YMBCrossCorrel), depends_on='data.input_change')
