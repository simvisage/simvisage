...
class LCCReaderInfoCAD(LCCReader):
    ...
    def read_geo_data(self, f_name):
        ...
        as text delimiter.
        '''
        geo_dir = os.path.join(self.data_dir, 'state_data', 'geo_data')
        node_file = os.path.join(geo_dir, 'KnotenKoordinaten.txt')        
        elem_file = os.path.join(geo_dir, 'Elemente.txt')        
        thic_file = os.path.join(geo_dir, 'Querschnittswerte.txt')
        
        node_arr = np.loadtxt(node_file)
        elem_arr = np.loadtxt(elem_file)
        thic_arr = np.loadtxt(thic_file) 

        midnode_arr = np.average(node_arr[elem_arr], axis = 1)
        
        X, Y, Z = midnode_arr
        
        thickness = thic_arr[elem_arr[:, 6]]
        
        return  {'elem_no':elem_arr[:, 0],
                 'X':X, 'Y':Y, 'Z':Z,
                 'thickness':thickness }
