...
#        print 'x', x
        ...
        z_c_i_arr = z_t_i_arr
        #z_c_i_arr = np.hstack(z_t_i_array, 0)
        eps_t_i_arr = eps_t / (thickness - x) * (z_c_i_arr - x)
        # use a ramp function to consider only negative strains
        eps_c_i_arr = (-np.fabs(eps_t_i_arr) + eps_t_i_arr) / 2.0
        
        #stress at the outer edge of the compression zone
#        eps_c_0 = eps_t / (thickness - x) * (0 - x)
#        eps_c_i_arr = np.hstack (eps_c_i_arr, eps_c_0)
#        
#        #exact development of eps_c_i_arr
#        z_c_i_arr =np.arange(x,0,0.01)
#        eps_c_i_arr = eps_t / (thickness - x) * (z_c_i_arr - x)
...
stress_state_cs = Trait('calculation_flexion',
                            {'calculation_flexion' : ('case_calculation_flexion'),                                                    
                                        
                             'calculation_compression' :('case_calculation_compression') 
                                                         
                             'calculation_tension' : ('case_calculation_tension')}    
                            
                            
                            def case_calculation_flexion( self, u ):
...
#        a = self.a
        ...
        # strain at the height of each reinforcement layer [-]:
        eps_t_i_arr = eps_t - (eps_t - eps_c ) / thickness*(thickness-z_t_i_arr)
        eps_fail = eps_t_i_arr[0]
#        sig_fail = 1216
#        b = (sig_fail - a * eps_fail **2) / eps_fail
#        eps = np.arange(0, eps_fail, 0.001) 
...
#
        ...
        return  eps_t_i_arr, mfn_line_array

def get_f_i_arr( self, u ):
        '''tensile force at the height of each reinforcement layer [kN]:
        '''
         eps_t_i_arr, layer_response = self.layer_response( u )
