...
class LSArrayAdapter (TabularAdapter):
    ...
    def _get_columns(self):
        # print 'GETTING COLUMNS', self.object.columns, self.object,
...
class LS(HasTraits):
    ...
    #
    columns = Property(
        List, depends_on='show_geo_columns, show_state_columns, show_ls_columns')
    ...
    def _plot_fired(self):
        ...
        # if this information is available (distinguished by the specific
        # Reader)
        r.plot_col(
            mlab, plot_col, gd, state_data=sd, warp_factor=self.warp_factor)
...
class SLS(LS):
    ...
    def _get_ls_values(self):
        ...
        eta_tot = ndmax(
            hstack([eta_tot_sig_up, eta_tot_sig_lo]), axis=1)[:, None]
    ...
    traits_view = View(VGroup(
        HGroup(Item(name='f_ctk', label='Tensile strength concrete [MPa]: f_ctk '),
               Item(
                   name='f_m', label='Flexural tensile trength concrete [MPa]: f_m ')
               ),
        VGroup(
            Include('ls_group'),
...
#                      self.sig1_lo[cond_up_cb].shape[0]
        ...
        if self.eval_mode == 'massivbau':
            ...
            f_t_sig_up[
                bool_arr] = n * (zs[bool_arr] + e[bool_arr]) / (zs[bool_arr] + zs[bool_arr])
        ...
        if self.eval_mode == 'princ_sig_level_1':
            ...
            f_t_sig_up[bool_arr] = self.sig1_up[
                bool_arr] * self.thickness[bool_arr] * 1000.
            ...
            f_t_sig_lo[bool_arr] = self.sig1_lo[
                bool_arr] * self.thickness[bool_arr] * 1000.
            ...
            eta_comp = ndmax(
                hstack([eta_comp_up, eta_comp_lo]), axis=1)[:, None]
        ...
        if self.eval_mode == 'eta_nm':
            ...
            if self.ls_table.k_alpha_min == True:
                ...
                n_Rdt_lo = n_Rdt_up = min(
                    self.n_0_Rdt, self.n_90_Rdt) * 0.707 * np.ones_like(n_Rdt_lo)
                m_Rd_lo = m_Rd_up = min(
                    self.m_0_Rd, self.m_90_Rd) * 0.707 * np.ones_like(m_Rd_lo)
            ...
            #
            # tensile force in direction of principle stress at upper side
            cond_nsu_ge_0 = self.n_sig_up >= 0.
            # tensile force in direction of principle stress at lower side
            cond_nsl_ge_0 = self.n_sig_lo >= 0.
            ...
            #
            # compressive force in direction of principle stress at upper side
            cond_nsu_lt_0 = self.n_sig_up < 0.
            # compressive force in direction of principle stress at lower side
            cond_nsl_lt_0 = self.n_sig_lo < 0.
    ...
    if eval_mode == 'massivbau':
        ...
        #
        plot_item_mpl = Item(
            name='f_Rtex_0', label='reinforcement strength per layer [kN/m]:  f_Rtex_0 ', style='readonly', format_str="%.1f")
        plot_item_mpt = Item(
            name='f_Rtex_90', label='reinforcement strength per layer [kN/m]:  f_Rtex_90 ', style='readonly', format_str="%.1f")
        ...
        plot_item_mpl = Item(name='f_Rtex_0', label='reinforcement strength per layer [kN/m]:  f_Rtex_0 ', style='readonly', format_str="%.1f"),\
            Item(name='sig_comp_0_Rd',
                 label='composit tensile strength [MPa]:  sig_comp_0_Rd ', style='readonly', format_str="%.1f")

        plot_item_mpt = Item(name='f_Rtex_90', label='reinforcement strength per layer [kN/m]:  f_Rtex_90 ', style='readonly', format_str="%.1f"),\
            Item(name='sig_comp_90_Rd',
                 label='composit tensile strength [MPa]:  sig_comp_90_Rd ', style='readonly', format_str="%.1f")
        ...
        plot_item_mpl = Item(name='n_0_Rdt', label='normal tensile strength [kN/m]:  n_0_Rdt ', style='readonly', format_str="%.1f"),\
            ...
            Item(
                name='m_0_Rd', label='bending strength [kNm/m]:  m_0_Rd ', style='readonly', format_str="%.1f")

        plot_item_mpt = Item(name='n_90_Rdt', label='normal tensile strength [kN/m]:  n_90_Rd ', style='readonly', format_str="%.1f"),\
            Item(name='n_Rdc', label='normal compressive strength [kN/m]:  n_0_Rdc ', style='readonly', format_str="%.1f"),\
            Item(
                name='m_90_Rd', label='bending strength [kNm/m]:  m_90_Rd ', style='readonly', format_str="%.1f")
...
class LSTable(HasTraits):
    ...
    def _get_princ_values_sig(self):
        ...
        #
        sig1_up = 0.5 * (sigx_up + sigy_up) + 0.5 * \
            sqrt((sigx_up - sigy_up) ** 2 + 4 * sigxy_up ** 2)
        sig2_up = 0.5 * (sigx_up + sigy_up) - 0.5 * \
            sqrt((sigx_up - sigy_up) ** 2 + 4 * sigxy_up ** 2)
        ...
        alpha_sig_up[bool_arr] = arctan(
            sigxy_up[bool_arr] / (sig2_up[bool_arr] - sigx_up[bool_arr]))
...
#        alpha_sig_up[ bool ] = 0.5 * arctan( 2 * sigxy_up[ bool ] / ( sigx_up[ bool ] - sigy_up[ bool ] ) )
        ...
        # transform the stresses at the lower face to the principle tensile
        # direction (1-direction) of the upper stresses
        sig1_lo_sig_up = 0.5 * (sigy_lo + sigx_lo) - 0.5 * (sigy_lo - sigx_lo) * cos(
            2 * alpha_sig_up) - sigxy_lo * sin(2 * alpha_sig_up)

        # transform moments and normal forces in the direction of the principal stresses (1-direction)
        #
        m_sig_up = 0.5 * (my + mx) - 0.5 * (my - mx) * \
            cos(2 * alpha_sig_up) - mxy * sin(2 * alpha_sig_up)
        n_sig_up = 0.5 * (ny + nx) - 0.5 * (ny - nx) * \
            cos(2 * alpha_sig_up) - nxy * sin(2 * alpha_sig_up)
        ...
        #
        sig1_lo = 0.5 * (sigx_lo + sigy_lo) + 0.5 * \
            sqrt((sigx_lo - sigy_lo) ** 2 + 4 * sigxy_lo ** 2)
        sig2_lo = 0.5 * (sigx_lo + sigy_lo) - 0.5 * \
            sqrt((sigx_lo - sigy_lo) ** 2 + 4 * sigxy_lo ** 2)
        ...
        alpha_sig_lo[bool_arr] = arctan(
            sigxy_lo[bool_arr] / (sig2_lo[bool_arr] - sigx_lo[bool_arr]))
...
#        alpha_sig_lo[ bool ] = 0.5 * arctan( 2 * sigxy_lo[ bool ] / ( sigx_lo[ bool ] - sigy_lo[ bool ] ) )
        ...
        #
        sig1_up_sig_lo = 0.5 * (sigy_up + sigx_up) - 0.5 * (sigy_up - sigx_up) * cos(
            2 * alpha_sig_lo) - sigxy_up * sin(2 * alpha_sig_lo)

        # transform moments and normal forces in the direction of the principal stresses (1-direction)
        #
        m_sig_lo = 0.5 * (my + mx) - 0.5 * (my - mx) * \
            cos(2 * alpha_sig_lo) - mxy * sin(2 * alpha_sig_lo)
        n_sig_lo = 0.5 * (ny + nx) - 0.5 * (ny - nx) * \
            cos(2 * alpha_sig_lo) - nxy * sin(2 * alpha_sig_lo)
