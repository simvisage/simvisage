...
#     sig_flag = 'tex'
    ...
    for t in tests:
        ...
        if sig_flag == 'comp':
            ex_run.ex_type._plot_comp_stress_strain_asc(p, plot_analytical_stiffness=False, color='black', linewidth=1.3, linestyle='--', label='CAR-800-TU')
        if sig_flag == 'tex':
            ex_run.ex_type._plot_tex_stress_strain_asc(p, color='black', linewidth=1.3, linestyle='--', label='CAR-800-TU')
    ...
    for t in tests:
        ...
        ex_run = ExRun(ex_path)
        if sig_flag == 'comp':
            ex_run.ex_type._plot_comp_stress_strain_asc(p, xscale=1000., color='black', linewidth=1.3, linestyle='-.', label='ARG-1200-TU')
        if sig_flag == 'tex':
            ex_run.ex_type._plot_tex_stress_strain_asc(p, xscale=1000., color='black', linewidth=1.3, linestyle='-.', label='2D-09-12')
...
#        tests = ['TT-10g-3cm-0-TR-V2.DAT']#, 'TT-10g-3cm-0-TR-V3.DAT']
    ...
    for t in tests:
        ...
        ex_run = ExRun(ex_path)
        if sig_flag == 'comp':
            ex_run.ex_type._plot_comp_stress_strain_asc(p, xscale=1000., color='black', linewidth=1.3, linestyle='-', label='ARG-1200-TR')
        if sig_flag == 'tex':
            ex_run.ex_type._plot_tex_stress_strain_asc(p, xscale=1000., color='black', linewidth=1.3, linestyle='-', label='2D-01-08')
