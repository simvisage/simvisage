...
#        print 'fn_list', fn_list
        ...
        for fn_idx in range(0, time_step_stop, time_step_size):
            ...
            fname = os.path.join(
                self.data_dir, '%s%d.txt' % (basename, fn_idx))
...
#        print 'y_idx_t: ',y_idx_t
        ...
        # first two columns
        # use minimum and maximum indices of all time steps to derive the grid
        # size
        x_idx_min_t, x_idx_max_t = np.min(
            x_idx_t, axis=1), np.max(x_idx_t, axis=1)
        y_idx_min_t, y_idx_max_t = np.min(
            y_idx_t, axis=1), np.max(y_idx_t, axis=1)
...
#        print 't_idx',t_idx
        ...
        grid_mask_t[:, :, :] = True
        grid_mask_t[
            (t_idx.flatten(), x_idx_t.flatten(), y_idx_t.flatten())] = False
...
#        print 'facette_distance_y',facette_distance_y
    ...
    def _get_data_t_orig(self):
        ...
        select_idx_arr = (
            self.txy_idx[0], self.txy_idx[1], self.txy_idx[2], slice(None))
    ...
    #        y_vec_ = daf_0[10, -1, :3] - daf_0[10, 0, :3]
            ...
            #
            daf_new[:, :, :, 0] = daf_new[:, :, :, 0] * cos_xx_ + \
                daf_new[:, :, :, 1] * cos_yx_ + daf_new[:, :, :, 2] * cos_zx_
            daf_new[:, :, :, 1] = daf_new[:, :, :, 0] * cos_xy_ + \
                daf_new[:, :, :, 1] * cos_yy_ + daf_new[:, :, :, 2] * cos_zy_
            daf_new[:, :, :, 2] = daf_new[:, :, :, 0] * cos_xz_ + \
                daf_new[:, :, :, 1] * cos_yz_ + daf_new[:, :, :, 2] * cos_zz_

            # rotate the displacement values (ux,uy,uz)
            #
            daf_new[:, :, :, 3] = daf_new[:, :, :, 3] * cos_xx_ + \
                daf_new[:, :, :, 4] * cos_yx_ + daf_new[:, :, :, 5] * cos_zx_
            daf_new[:, :, :, 4] = daf_new[:, :, :, 3] * cos_xy_ + \
                daf_new[:, :, :, 4] * cos_yy_ + daf_new[:, :, :, 5] * cos_zy_
            daf_new[:, :, :, 5] = daf_new[:, :, :, 3] * cos_xz_ + \
                daf_new[:, :, :, 4] * cos_yz_ + daf_new[:, :, :, 5] * cos_zz_
    ...
    def _get_x_arr_avg(self):
        # use average to eliminate errors in measuring of single points (yields
        # a 1d-array)
    ...
    def _get_ux_t(self):
        ...
        ux_arr[t_idx_zeros, x_idx_zeros, y_idx_zeros] = ux_avg[
            t_idx_zeros, x_idx_zeros]
    ...
    def plot3d_points(self):
        ...
        # the missing facettes therefore are displayed in points3d as missing
        # elements (gaps)
...
#        m.points3d(self.x_arr_avg, self.y_arr_avg, self.z_arr, plot3d_var, mode = 'cube', colormap = "blue-red", scale_mode ='none', vmax = vmax)
        m.points3d(self.x_arr, self.y_arr, self.z_arr, plot3d_var,
                   mode='cube', colormap="blue-red", scale_mode='none', vmax=vmax)
...
#        m.scalarbar().position = [0., 0.]
        ...
        module_manager.scalar_lut_manager.scalar_bar_representation.maximum_size = np.array(
            [100000, 100000])
        module_manager.scalar_lut_manager.scalar_bar_representation.minimum_size = np.array(
            [1, 1])
        module_manager.scalar_lut_manager.scalar_bar_representation.position2 = np.array(
            [0.20603604, 0.16827586])
...
#        plot3d_var = getattr(self, 'crack_field_w')
        vmax = self.plot3d_var_[1]
        m.points3d(z_arr, self.x_arr, self.y_arr, plot3d_var,
                   mode='cube', colormap="blue-red", scale_mode='scalar', vmax=vmax)
...
#        plot3d_var = getattr(self, 'd_ux_w')
        ...
        scene.scene.camera.position = [
            616.49832063929375, 638.29074243238438, 514.06081220962164]
        scene.scene.camera.focal_point = [
            11.259753942489624, 11.990119934082031, 9.7502956390380859]
        scene.scene.camera.view_angle = 30.0
        scene.scene.camera.view_up = [
            0.79246046934594205, -0.54446721176015089, -0.27488517574095594]
        scene.scene.camera.clipping_range = [
            595.49259262137014, 1526.1362976999562]
...
#        m.surf(self.x_arr, self.y_arr, plot3d_var, vmax = vmax, mask = mask_arr, warp_scale = warp_scale)
        m.surf(self.x_arr_avg, self.y_arr_avg, plot3d_var,
               vmax=vmax, mask=mask_arr, warp_scale=warp_scale)
...
#        m.axes()
        ...
        if self.warp_scale == 1:
            ...
            scene.scene.camera.position = [-319.07443227647047, -
                                           469.29101319337502, 434.40818470843612]
            scene.scene.camera.focal_point = [
                -66.983721840860625, -79.447548413824947, 1.1328650920308507]
            scene.scene.camera.view_angle = 30.0
            scene.scene.camera.view_up = [
                0.49290441759866288, 0.48449293877207339, 0.72271144130401255]
            scene.scene.camera.clipping_range = [
                325.28180250321782, 995.97136098229157]
            scene.scene.camera.compute_view_plane_normal()
            scene.scene.render()

        # predefine the position of the scalarbar
        #
        module_manager = engine.scenes[0].children[
            0].children[0].children[0].children[0]
        module_manager.scalar_lut_manager.scalar_bar_representation.minimum_size = np.array(
            [1, 1])
        module_manager.scalar_lut_manager.scalar_bar_representation.position2 = np.array(
            [0.27780226, 0.11638842])
        module_manager.scalar_lut_manager.scalar_bar_representation.position = np.array(
            [0.59707606, 0.16105125])
        module_manager.scalar_lut_manager.scalar_bar_representation.maximum_size = np.array(
            [100000, 100000])
        m.show()
...
if __name__ == '__main__':
    ...
    aramis_dir = os.path.join(
        simdb.exdata_dir, 'tensile_tests', 'dog_bone', '2012-04-12_TT-12c-6cm-0-TU_SH4', 'ARAMIS')
    home_dir = os.path.expanduser('~')
    #aramis_dir = os.path.join(home_dir, 'Trash')
