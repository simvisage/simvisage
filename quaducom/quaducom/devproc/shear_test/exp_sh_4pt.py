#-------------------------------------------------------------------------
#
# Copyright (c) 2016, IMB, RWTH Aachen.
# All rights reserved.
#
...
# edited on June 20, 2016 by: jb

#Todo: add functions to plot displacement F1 and F2 under the load introduction 1 and 2, respectively
...
class ExpSH4PT(ExType):
    '''Experiment: Shear Test Four Point
    '''
#    label = Str('four point bending test')
    ...
    age = Int(260, unit='d', input=True, table_field=True,
              auto_set=False, enter_set=True)
    loading_rate = Float(1.0, unit='mm/min', input=True, table_field=True,
                         auto_set=False, enter_set=True)

    # additional own weight of load introduction
    weight_load_introduction = Float(0.59, unit='kN', input=True, table_field=True,
                                     auto_set=False, enter_set=True)
    ...
    def process_source_data(self):
        ...
        super(ExpSH4PT, self).process_source_data()
...
if __name__ == '__main__':
    ...
    ex = ExRunClassExt(klass=ExpSH4PT)
    ex.configure_traits()
