...
from ibvpy.api import \
    ...
    BCDof, BCSlice, FERefinementGrid, FEDomain
...
class SimST(IBVModel):
    ...
    #
    geo_st_plain_concrete = Property(
        Instance(GeoST), depends_on='+ps_levels, +input')
    ...
    # used in the simulation of the slab test and within the calibration
    # procedure.
...
#    E_specmn_mats = Float(input=True)
    ...
    def _get_specmn_mats(self):
        ...
        specmn_mats = MATS2D5MicroplaneDamage(
            ...
            # E=self.E_m,  # #@todo: only available if this parameter is
...
#         fets = FETS3D8H20U(mats_eval=self.supprt_mats)
    ...
    specmn_fe_grid = Property(
        Instance(FEGrid), depends_on='+ps_levels, +input')
    ...
    def _get_specmn_fe_grid(self):
        ...
        if self.geo_st_flag:
            fe_grid = FEGrid(coord_min=(0.0, 0.0, 0.0),
                             ...
                             shape=(
                                 self.shape_xy, self.shape_xy, self.shape_z),
                             level=self.specmn_fe_level,
                             geo_transform=self.geo_st,
                             fets_eval=self.specmn_fets)
        else:
            # total effective length = 1.25m - 2*0.05m (from center supports to
            # center support)
            width_supprt = 0.1
            ...
            fe_grid = FEGrid(coord_min=(0.0, 0.0, 0.0),
                             coord_max=(
                                 length_quarter_eff, length_quarter_eff, self.thickness),
                             shape=(
                                 self.shape_xy, self.shape_xy, self.shape_z),
                             level=self.specmn_fe_level,
                             fets_eval=self.specmn_fets)
    ...
    plain_concrete_fe_grid = Property(
        Instance(FEGrid), depends_on='+ps_levels, +input')
    ...
    def _get_elstmr_fe_grid(self):
        ...
        #
            ...
            fe_grid = FEGrid(coord_min=(elstmr_min, elstmr_min, self.thickness),
                             coord_max=(
                                 elstmr_max, elstmr_max, self.thickness + self.thickness_elstmr),
                             level=self.elstmr_fe_level,
                             shape=(self.shape_R, self.shape_R, 1),
                             fets_eval=self.elstmr_fets)
        return fe_grid
    ...
    if supprt_flag:
        ...
        def _get_supprt_fe_grid(self):
            return FEGrid(coord_min=(0, 0, 0),
                          ...
                          #
                          shape=(
                              self.shape_supprt_xy, self.shape_supprt_xy, 1),
    ...
    def _get_tloop(self):
        ...
        #
        bc_symplane_yz = BCSlice(
            var='u', value=0., dims=[0], slice=specmn[-1, :, :, -1, :, :])
        bc_symplane_xz = BCSlice(
            var='u', value=0., dims=[1], slice=specmn[:, -1, :, :, -1, :])

        if self.plain_concrete_flag:
            # symmetry planes of the plain concrete layer
            #
            bc_symplane_yz_plain_concrete = BCSlice(
                var='u', value=0., dims=[0], slice=plain_concrete[-1, :, :, -1, :, :])
            bc_symplane_xz_plain_concrete = BCSlice(
                var='u', value=0., dims=[1], slice=plain_concrete[:, -1, :, :, -1, :])
...
#                                   link_coeffs = [1.])
        ...
        if self.supprt_flag:
            ...
            if self.plain_concrete_flag:
                ...
                link_pc_sprt_z = BCSlice(var='u', value=0., dims=[2],
                                         slice=plain_concrete[
                                             0:self.shape_supprt_xy, 0:self.shape_supprt_xy, 0, :, :, 0],
            ...
            # print 'supprt_dof_z used for internal force tracing ',
            # supprt_dof_z
            ...
            #
            support_000 = BCSlice(var='u', value=0., dims=[2], slice=specmn[
        ...
        #
        w_max = self.w_max
# w_max = Float( -0.020, auto_set = False, enter_set = True, input = True)
# # [m]
        ...
        if self.elstmr_flag:
            ...
            if self.geo_st_flag:
                ...
                link_el_sp = BCSlice(var='u', value=0., dims=[2],
                                     slice=elstmr[:, :, 0, :, :, 0],
                                     link_slice=specmn[-self.shape_R:, -
                                                       self.shape_R:, -1, :, :, -1],
                                     link_dims=[2],
                                     link_coeffs=[1.])
            else:
...
#            time_function = MFnLineArray(xdata = [0.0, 0.2, 0.4, 1.0], ydata = [0.0, 0.2, 0.75, 1.0])
        ...
        #
            ...
            if self.geo_st_flag:
                ...
                #
                nodes_line_Rx = specmn[-self.shape_R:, -
                                       self.shape_R, -1, :, 0, -1]
                nodes_line_Ry = specmn[-self.shape_R, -
                                       self.shape_R:, -1, 0, :, -1]
                bc_w_Rxline = BCSlice(
                    var='u', value=w_max, dims=[2], slice=nodes_line_Rx)
                bc_w_Ryline = BCSlice(
                    var='u', value=w_max, dims=[2], slice=nodes_line_Ry)
                ...
                # array as it appears in two adjacent elements of the same
...
#                                            transform_y = '-4 * 1000 * y')
        ...
        if self.supprt_flag:
            if self.plain_concrete_flag:
                bcond_list += [link_sp_pc_xyz, link_pc_sprt_z, sym_support_link,
                               bc_symplane_yz_plain_concrete, bc_symplane_xz_plain_concrete]
            else:
        ...
        ts = TS(
            ...
            rtrace_list=[
                ...
                # value in phi_mtx for 1st principle damage
                # direction
    ...
    def peval(self):
        ...
        F_max = np.max(self.f_w_diagram_center.trace.ydata)

        u_center_top_z = U[self.center_top_dofs][0, 0, 2]
        return np.array([u_center_top_z, F_max],
                        dtype='float_')
...
class SimSTDB(SimST):
    ...
    def _get_damage_function(self):
        # print'XXX',
        # [self.ccs_unit_cell_ref.damage_function_list[i].calibration_test for
        # i in range(14)]
    ...
    #
    phi_fn_class = Enum(PhiFnGeneral, [
                        PhiFnGeneral, PhiFnGeneralExtended, PhiFnGeneralExtendedExp], input=True,)
    phi_fn = Property(
        Instance(phi_fn_class), depends_on='phi_fn_class, input_change, +ps_levels')
...
if __name__ == '__main__':
    ...
    sim_model = SimSTDB(
        ...
        #                         shape_z = 3,
                         ...
                         tmax=0.4
        #                         tstep = 0.05
        # tstep = 0.025 # tmax / 34 steps with tmax = 0.85 corresponding to 1
        # mm / tstep
    )
...
#        sim_model.tloop.eval()
    ...
    if do == 'validation':
        ...
        param_key = ccs_unit_cell_key + '_' + calibration_test + \
            'L=%g h=% sxy= %g Em=%g nu=%g tol=%g' % (
                length, thickness, shape_xy, E_m, nu, tolerance)
...
#            ex_run.ex_type._plot_force_center_deflection_smoothed(p)
        ...
        # ST-6c-2cm-TU_bs2
        #
        ex_path = join(simdb.exdata_dir, 'slab_tests',
