...
class SCM( HasTraits ):
    ...
    E_f = Float( 120e+3, auto_set = False, enter_set = True, # [N/mm^2]
                 desc = 'E-Modulus of the fiber [MPa]',
                 modified = True )

    E_m = Float( 34e+3, auto_set = False, enter_set = True, # [N/mm^2]
    ...
    def _get_delta_final( self ):
        return self.sigma_ref * ( self.V_m * self.r ) / ( self.V_f * 2 * self.tau )
    ...
    # mean and m as shape parameter
    scale_sigma_c = Property( Float, depends_on = 'sigma_mu, m, E_m, E_f, reinf_ratio.rho, orientation' )
    @cached_property
    ...
    csf = Property( depends_on = 'm, sigma_mu, sigma_ref, Pf, sigma_fu, orientation, E_f, E_m' )
    @cached_property
    def _get_csf( self ):
