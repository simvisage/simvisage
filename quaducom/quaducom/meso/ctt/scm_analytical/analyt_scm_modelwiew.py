...
class SCMModelView ( ModelView ):
    ...
    # switches between stresses with respect to area of composite and reinforcement, respectively
    ...
    def refresh( self ):
        ...
        if self.stresses == 'composite':
            ...
            if self.csf:
                axes.plot( epsilon_csf, sigma_csf, lw = 1, color = 'green' , linestyle = 'dotted' )
            ...
            axes.plot( epsilon_c_arr[[0, -1]], sigma_fiber / scm.rho, lw = 2, color = 'grey' )
            legend = ['reinforcement', 'reference stiffness']
            if self.csf:
                axes.plot( epsilon_csf, sigma_csf / scm.rho, lw = 1, color = 'green' , linestyle = 'dotted' )
                legend.append( 'strain at $CS_f$' )
            axes.legend( legend, 'upper left' )
