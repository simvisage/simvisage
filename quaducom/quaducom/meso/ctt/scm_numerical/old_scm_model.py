...
class SCM(HasTraits):
    ...
    def evaluate(self):
        ...
        while np.any(self.sigma_m > self.matrix_strength):
            ...
            self.sort_cbs()
            e_arr = make_ogrid([np.arange(len(self.x_arr)), np.arange(len(self.load_sigma_c))])
            mlab.surf(e_arr[0], e_arr[1], self.sigma_m)
            mlab.surf(e_arr[0], e_arr[1], self.matrix_strength)
            mlab.show()
