...
class RepresentativeCB(HasTraits):
    ...
    length = Float
    CB_model_view = Property(
        Instance(CompositeCrackBridgeView), depends_on='CB_model')
    ...
    def w_x_res(self, w_arr, ll, lr):
        ...
        for w in w_arr:
            ...
            if self.CB_model_view.sigma_c > sigma_c_lst[-1]:
                ...
                x_i = np.hstack(
                    (-self.length - 1e-1, self.CB_model_view.x_arr, self.length + 1e-1))
                epsm_i = np.hstack((self.CB_model_view.epsm_arr[
                                   0], self.CB_model_view.epsm_arr, self.CB_model_view.epsm_arr[-1]))
                epsf_i = np.hstack((self.CB_model_view.epsf_arr[
                                   0], self.CB_model_view.epsf_arr, self.CB_model_view.epsf_arr[-1]))
        ...
        w_interpolator = MFnLineArray(
            xdata=np.array(sigma_c_lst), ydata=np.array(w_lst))
    ...
    def interpolate_epsm(self, Ll, Lr, sigma_c, x_arr):
        ll_idx_high, lr_idx_high, ll_idx_low, lr_idx_low = self.get_BC_idxs(
            Ll, Lr)
        epsm_interpolator_lst = self.interpolator_lists[
            1][ll_idx_high, lr_idx_high]
        ...
        if sigma_c > sigc[-1]:
            ...
            coeff_high = (
                sigma_c - sigc[sigc_low]) / (sigc[sigc_high] - sigc[sigc_low])
            if Lr >= Ll:
    ...
    def interpolate_epsf(self, Ll, Lr, sigma_c, x_arr):
        ll_idx_high, lr_idx_high, ll_idx_low, lr_idx_low = self.get_BC_idxs(
            Ll, Lr)
        epsf_interpolator_lst = self.interpolator_lists[
            3][ll_idx_high, lr_idx_high]
    ...
    def interpolate_w(self, Ll, Lr, sigma_c):
        ...
        ll_idx_high, lr_idx_high, ll_idx_low, lr_idx_low = self.get_BC_idxs(
            Ll, Lr)
        ll_low, ll_high, lr_low, lr_high = self.BC_range[ll_idx_low], self.BC_range[
            ll_idx_high], self.BC_range[lr_idx_low], self.BC_range[lr_idx_high]
        # evaluating nodal values / note that w2 = w3
        w1 = self.interpolator_lists[2][
            ll_idx_low, lr_idx_low].get_values(np.array([sigma_c]))
        w2 = self.interpolator_lists[2][
            ll_idx_low, lr_idx_high].get_values(np.array([sigma_c]))
        w3 = self.interpolator_lists[2][
            ll_idx_high, lr_idx_low].get_values(np.array([sigma_c]))
        w4 = self.interpolator_lists[2][
            ll_idx_high, lr_idx_high].get_values(np.array([sigma_c]))
