    ...
    def F(self, dems, amin):
        ...
        for i, mask in enumerate(self.sorted_masks):
            ...
            if i == 0:
                ...
                depsf2 = depsf0[idx]
                a1 = np.exp(F[self.sorted_masks[i - 1]]
                            [idx] / 2. + np.log(amin))
                p = depsf2 - depsf1
                q = depsf1 + demsi[0]
    ...
    def profile(self, iter_damage, Lmin, Lmax):
        ...
        if Lmin < a1[0] and Lmax < a1[0]:
            ...
            amin = -Lmin + np.sqrt(4 * Lmin ** 2 * p ** 2 - 4 * p * emLmin * Lmin +
                                   4 * p * umLmin - 2 * p * Lmin ** 2 * depsfLmin + 2 * p * self.w) / p
            C = np.log(amin ** 2 + 2 * amin * Lmin - Lmin ** 2)
            a2 = (np.sqrt(2 * Lmin ** 2 +
                          np.exp(F + C - F[idx1])) - Lmin)[idx1:]
...
if __name__ == '__main__':
    ...
    ccb.damage
    plt.plot(np.zeros_like(ccb._epsf0_arr),
             ccb._epsf0_arr, 'ro', label='maximum')
    for i, depsf in enumerate(ccb.sorted_depsf):
        epsf_x = np.maximum(
            ccb._epsf0_arr[i] - depsf * np.abs(ccb._x_arr), ccb._epsm_arr)
